...
int main(int argc, char **argv) {
    ...
    strcpy(production[2], "R=#");      // # represents epsilon
        ...
        for(j = 2; j < 10; j++) {
            if(production[i][j] == c) {
                ...
                if(production[i][j+1]=='\0' && c!=production[i][0]) {
                    // Calculate the follow of the Non-Terminal in L.H.S.
...
void followfirst(char c, int c1, int c2) {
    ...
    if(!(isupper(c)))
        ...
        // Including the First set of the Non-Terminal in Follow
        while(calc_first[i][j] != '!') {
            if(calc_first[i][j] != '#') {
                ...
                if(production[c1][c2] == '\0') {
                    ...
                    // Recursion in case we encounter epsilon
